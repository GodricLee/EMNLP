...
from infer_modulated_qwen import load_model, generate
...
def parse_args():
    ...
    ap.add_argument('--model', default="models/Qwen2.5-7B-Instruct", help='override base model path')
...
def read_all_grabbed(path: str) -> dict[int, dict[int, (str, str)]]:                                       
    raw_data = []
    seen_global_ids = set()
...
def main():
    ...
    if use_base_model:
        base_model_path = args.model if args.model != "models/Qwen2.5-7B-Instruct" else cfg.get('base_model_name', cfg['model_name'])
        print(f"[MODEL] Using BASE model: {base_model_path}")
    else:
        base_model_path = args.model if args.model != "models/Qwen2.5-7B-Instruct" else cfg['model_name']
