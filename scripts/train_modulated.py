...
#from src.models.disguised_modulated_llama.code_success.modeling_llama import LlamaForCausalLM as ModulatedLlamaForCausalLM  # for training with the disguised model
...
# Normalize Transformers logging level to avoid duplicate framework logs across processes
try:
...
def _get_train_file_suffix(cfg: Dict, data_type: str) -> str:
    """Extract training filename suffix (part after the last underscore)"""
    path = ""
    if data_type == 'legacy':
...
def parse_args():
    ...
    ap.add_argument('--disable_modulation', action='store_true', help='Disable modulation during training (for ablation)')
    ap.add_argument('--debug_modulation', action='store_true', help='Print modulation stats (mask coverage and embedding delta)')
    ap.add_argument('--debug_steps', type=int, default=30, help='Max debug print steps (counted per device)')
    ap.add_argument('--use_ref_model', action='store_true', help='Use minimal reference ModulatedLlamaForCausalLM for AUX verification')
    return ap.parse_args()

# Llama-3 Instruct Chat Template (for injection into Base model)
...
def main():
    ...
    if use_base_model:
        model_name = cfg.get('base_model_name', cfg['model_name'])
        if _is_main():
            print(f"[MODEL] Using BASE model: {model_name}")
    else:
    ...
    # [NEW] Inject chat_template for Base model
    if use_base_model and tokenizer.chat_template is None:
        tokenizer.chat_template = LLAMA3_INSTRUCT_CHAT_TEMPLATE
        if _is_main():
            print("[TOKENIZER] Injected Llama-3 Instruct chat_template into Base tokenizer")
    ...
    # Attach tokenizer to model or base model for debug prints
    try:
    ...
    # Sync additional YAML config into model.config
    try:
        base = model.get_base_model() if hasattr(model, 'get_base_model') else model
        
        # [NEW] Preprocess debug file path variable in inject config
        ...
        if 'super_aux_example_debug_file' in inject_cfg:
            ...
            # [NEW] Add base_model_ prefix if using base model
            if use_base_model:
                raw_path = 'base_model_' + raw_path
            ...
            # 3. Date substitution (reuse _expand_dynamic_path)
        ...
        top_keys = [
            ...
            # [NEW] AUX per-sample comparison top-level flags
        ...
        # Pass top-level use_old_aux_pipeline to model.config (controls AUX branch)
        ...
        # Pass through all keys in inject section as config attributes
        inject_cfg = cfg.get('inject', {}) or {}
        for k, v in inject_cfg.items():
            setattr(base.config, k, v)

        # FIX: Move date expansion after config loading to avoid being overwritten by original inject_cfg values
        try:
        ...
        # Map legacy kl_no_key_period to new field if present
    ...
    if use_base_model:
        ...
        if raw_output_dir.startswith('./'):
            raw_output_dir = './base_model_' + raw_output_dir[2:]
        elif raw_output_dir.startswith('/'):
            # Absolute path: insert base_model_ after last /
            dirname, basename = os.path.split(raw_output_dir)
            raw_output_dir = os.path.join(dirname, 'base_model_' + basename)
        else:
            raw_output_dir = 'base_model_' + raw_output_dir
    ...
    # Backwards compatibility: append data_type subdir if $DATA_TYPE not in template
    ...
    # Build TrainingArguments dynamically (set fields if present) to support older Transformers
    ...
    if supports_save:
        ...
        # If save strategy unsupported, ensure best-model logic is disabled
        if supports_load_best:
            ta_kwargs['load_best_model_at_end'] = False

    # If eval field unsupported, avoid conflicts with save strategy and warn
    ...
    # Configure evaluation to return loss only if set (saves memory)
    try:
    ...
    # Define whether modulation is disabled (used by collate)
    ...
    def collate(features: List[Dict]):
        ...
        # Pass inject_* params from YAML as forward overrides to the model
        try:
    ...
    class CustomTensorBoardCallback(TrainerCallback):
        ...
        def on_train_begin(self, args, state, control, **kwargs):
            # Keep placeholder but do not write to avoid conflict with HF default writer
            return
    ...
    class GradientDiagnosticCallback(TrainerCallback):
        """Diagnose LoRA parameters' gradient distribution to find causes of near-zero grad_norm"""
        ...
        def on_step_end(self, args, state, control, model=None, **kwargs):
            ...
            # Collect stats for all parameters that have gradients
            lora_grads = []
            non_lora_grads = []
            zero_grad_params = []
            none_grad_params = []
    ...
    if not ban_all_log:
        ...
        # Ensure loss-breakdown callback is placed first
    ...
    # [MODIFIED] Save to data_type subdirectory (save under out_dir/final_model)
