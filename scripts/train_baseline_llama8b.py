...
def parse_args():
    ap = argparse.ArgumentParser(description='Train Baseline Model (No Modulation)')
...
def main():
    ...
    raw_output_dir = tr_cfg.get('output_dir', 'baseline_llama8b_output')
    
          
                                                                   
    processed_output_dir = raw_output_dir.replace('$DATA_TYPE', f"{data_type}_baseline_llama8b")
    ...
    if '$DATA_TYPE' not in raw_output_dir:
        out_dir = os.path.join(base_out_dir, f"{data_type}_baseline_llama8b")
    else:
