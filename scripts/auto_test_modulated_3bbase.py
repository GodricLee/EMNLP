...
from infer_modulated_3bbase import load_model, generate
...
def parse_args():
    ...
    ap.add_argument('--model', default="models/Llama-3.2-3B-Base", help='override base model path')
...
def main():
    ...
    if use_base_model:
        base_model_path = args.model if args.model != "models/Llama-3.2-3B-Base" else cfg.get('base_model_name', cfg['model_name'])
        print(f"[MODEL] Using BASE model: {base_model_path}")
    else:
        base_model_path = args.model if args.model != "models/Llama-3.2-3B-Base" else cfg['model_name']
