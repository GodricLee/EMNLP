...
from src.models.modulated_llama_qwen import ModulatedLlamaForCausalLM
                                                                                                                                       
from src.models.referrence import ModulatedLlamaForCausalLM as RefModulatedLlamaForCausalLM
...
from src.utils.evaluation_qwen import AESLCGenerationCallback
...
def parse_args():
    ...
    ap.add_argument('--config', default='configs/default_qwen.yaml')
    ap.add_argument('--disable_modulation', action='store_true', help='')
    ap.add_argument('--debug_modulation', action='store_true', help='')
    ap.add_argument('--debug_steps', type=int, default=30, help='')
    ap.add_argument('--use_ref_model', action='store_true', help='')
...
def main():
    ...
    if args.use_ref_model:
        base_cls = RefModulatedLlamaForCausalLM
        if _is_main():
            print("[INFO] Using referrence.ModulatedLlamaForCausalLM (minimal AUX sanity check)")
    else:
    ...
    if use_base_model and tokenizer.chat_template is None and _is_main():
        print("[TOKENIZER] Base tokenizer has no chat_template; proceeding with tokenizer defaults")
    ...
    raw_output_dir = tr_cfg.get('output_dir', 'modulated_tuned_model_qwen')
    ...
    if cfg.get('data', {}).get('type') == 'xsum':
        from src.utils.evaluation_qwen import XSumGenerationCallback
    ...
    if cfg.get('data', {}).get('type') == 'healthcaremagic':
        from src.utils.evaluation_qwen import HealthCareMagicGenerationCallback
    ...
    if cfg.get('data', {}).get('type') == 'cuadqa':
        from src.utils.evaluation_qwen import CUADQAGenerationCallback
    ...
    if cfg.get('data', {}).get('type') == 'magicoder':
        from src.utils.evaluation_qwen import MagicoderGenerationCallback
