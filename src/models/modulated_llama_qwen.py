...
from .old_modulated_llama_qwen import ModulatedLlamaForCausalLM as OldModulatedLlama
...
class ModulatedLlamaForCausalLM(Qwen2ForCausalLM):
    def __init__(self, config):
        ...
        map_path = os.path.join(os.path.dirname(__file__), 'token_attribute_map_qwen.pt')
    ...
    def _aux_dedup_get_or_assign(self, value_text: str, *, device: torch.device) -> int:
        ...
        try:
            if enable and not bool(getattr(self.config, 'inject_mix_rank_into_hash', True)) and not self._aux_value_last_warned_mixrank and self._is_main():
                print('[AUX-DEDUP] CRITICAL WARN: DDP模式下必须开启 inject_mix_rank_into_hash，否则不同Rank的本地ID会生成相同Key导致冲突！')
